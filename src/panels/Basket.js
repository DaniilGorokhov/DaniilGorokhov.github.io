...
const Basket = ({ match: { params: { areaId, itemId }}, foodAreas, order }) => {
  const [ faster, setFaster ] = useState( JSON.parse(localStorage.getItem("isFaster")) || true);
  const [ time, setTime ] = useState( localStorage.getItem("deliveryTime") || '');
  const [ selfService, setSelfService ] = useState( JSON.parse(localStorage.getItem("isSelfService")) || false);
  ...
  return (
    <div className="Place">
      ...
      <div className="Place__products-wrapper">
        ...
        <Link
            onClick={() => {
                localStorage.setItem("isFaster", faster);
                localStorage.setItem("deliveryTime", time);
                localStorage.setItem("isSelfService", selfService);
            }}
